...
/* 수락 버튼과 거절 버튼이 동일한 라인에 있을 때 간격 설정 */
.action-buttons button {
    margin: -7 5px; /* 버튼 간의 간격 */
}
					...
					function() {
										...
										success : function(data) {
											...
											if (data.error) {
												...
												if (data.list.length === 0) {
															...
															.each(
																	...
																	function(
																		...
																		console
																				.log(
																						"unlike_id:",
																						friend.unlike_id); // 각 친구의 unlike_id 로그 찍기
																		console
																				.log(
																						"friend.id:",
																						friend.id); // 각 친구의 id 로그 찍기
																		var imageSrc = (friend.friend_image && friend.friend_image
																				.trim()) ? '/photo/'
																				+ friend.friend_image
																				: 'resources/img/common/profile.png';
													...
													$('.no-friends-message')
															.hide(); // 데이터가 있을 경우 메시지 숨김
													// 페이지네이션 생성
													setupPagination(
															data.totalCount,
															page);
													$('.pagination-container')
															.show(); // 데이터가 있을 경우 페이지네이션 표시
												}
							...
							$
									.ajax({
										...
										success : function(response) {
											if (response.success) {
												alert(action === 'accept' ? '친구 요청을 수락했습니다.'
														: '친구 요청을 거절했습니다.');
												$(
														'.card[data-id="'
																+ unlikeId
						...
						$(document)
								.on(
										...
										function() {
											document
													.getElementById("profilePopup").style.display = "none";
										});
